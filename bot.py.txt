...
async def slot(interaction: discord.Interaction, bet: int):
    ...
    if bet <= 0 or bet > user_coins[user_id]:
        await interaction.response.send_message("⚠️ 賭け金が無効か、コインが足りないよ！", ephemeral=True)
        return
    ...
    if user_super_charm_active.get(user_id, False):
        ...
        await interaction.followup.send(f"🌟 スーパーラッキーチャーム発動！ジャックポット！{winnings}コイン獲得！")
    ...
    if roll[0] == roll[1] == roll[2]:
        winnings = bet * 5
        user_coins[user_id] += winnings
        await interaction.followup.send(f"🎉 ジャックポット！{winnings}コイン獲得！")
    elif roll[0] == roll[1] or roll[1] == roll[2] or roll[0] == roll[2]:
        winnings = bet * 2
        user_coins[user_id] += winnings
        await interaction.followup.send(f"✨ ナイス！{winnings}コイン獲得！")
    else:
        user_coins[user_id] -= bet
        await interaction.followup.send(f"😢 はずれ！{bet}コイン失ったよ…")

    save_json(COIN_FILE, user_coins)
    await interaction.followup.send(f"💰 現在のコイン残高: {user_coins[user_id]}")

# 🧧 ラッキーチャーム購入
@bot.tree.command(name="buycharm", description="ラッキーチャームを購入するよ")
async def buycharm(interaction: discord.Interaction):
    user_id = str(interaction.user.id)
    if user_charms.get(user_id, False):
        await interaction.response.send_message("🧧 すでにお守りを持ってるよ！", ephemeral=True)
        return
    ...
    save_json(COIN_FILE, user_coins)
    save_json(CHARM_FILE, user_charms)
    await interaction.response.send_message("🧧 ラッキーチャームを購入したよ！")

# 🌟 スーパーラッキーチャーム使用
@bot.tree.command(name="usesupercharm", description="スーパーラッキーチャームを使うよ")
...
# 🎁 ミステリーボックス購入
@bot.tree.command(name="buybox", description="ミステリーボックスを購入して開けるよ！")
...
# 📅 デイリーボーナス
@bot.tree.command(name="daily", description="今日のデイリーボーナスを受け取るよ！")
async def daily(interaction: discord.Interaction):
    user_id = str(interaction.user.id)
    weekday = datetime.utcnow().weekday()  # 月曜=0, 日曜=6
    current_week = get_current_week()
    ...
    if weekday in daily_claims[user_id]["claimed"]:
        await interaction.response.send_message("📅 今日のボーナスはもう受け取ったよ！また明日ね！", ephemeral=True)
        return
    ...
    await interaction.response.send_message(
        f"🎁 {day_name}のデイリーボーナス！{bonus}コインをゲットしたよ！\n💰 現在のコイン残高: {user_coins[user_id]}"
    )
...
# 🏆 ランキングボーナス受け取り
@bot.tree.command(name="rankbonus", description="2週に1回のランキングボーナスを受け取るよ！")
...
async def balance(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"💰 あなたのコイン残高は {coins} コインだよ！")
